  ...
  return (
    <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }} className="trip-detail-hero overflow-hidden rounded-[1.75rem] border shadow-[0_2rem_4rem_rgba(0,0,0,.2)]">
      <div className="trip-detail-hero__media relative aspect-[16/9] min-h-[18rem] overflow-hidden sm:min-h-[25rem] lg:min-h-[32rem]">
        ...
        <div className="absolute left-5 top-5 rounded-full border border-white/20 bg-black/25 px-3 py-1.5 text-xs font-bold uppercase tracking-[.16em] text-white/80 backdrop-blur-md">{t("curatedJourneys")}</div>
        ...
        <div className="absolute inset-x-5 bottom-6 max-w-3xl"><p className="mb-2 text-sm font-bold uppercase tracking-[.18em] text-[#e0b873]">{t("curatedJourneys")}</p><h1 className="text-3xl font-bold leading-tight text-white sm:text-5xl">{title}</h1><p className="mt-3 line-clamp-3 max-w-2xl text-sm leading-6 text-white/70 sm:text-base">{description}</p><div className="mt-4 flex items-center gap-2 text-sm text-[#f0c979]"><FaStar /><strong>{trip.rating || "4.5"}</strong><span className="text-white/55">({trip.review_count ?? trip.reviews?.length ?? 0})</span></div></div>
      </div>

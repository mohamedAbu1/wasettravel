...
import { FaFacebookF, FaInstagram, FaTiktok, FaWhatsapp, FaGooglePlay, FaApple } from "react-icons/fa";
...
export default function HeroSection() {
  ...
  return (
    <section className={`relative h-[100vh] w-full overflow-hidden ${theme.background} ${theme.text}`}>
      {/* Background Image */}
      <div className="absolute inset-0">
        <AnimatePresence>
          ...
          >
            <Image src={images[index]} alt="WasetTravel Slide" fill priority className="object-cover" />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      >
        {[{ Icon: FaGooglePlay, link: "https://play.google.com" }, { Icon: FaApple, link: "https://apple.com" }].map(
          ({ Icon, link }, i) => (
            <motion.a
      ...
      <motion.div
        ...
        className="absolute left-10 md:top-1/2 -translate-y-1/2 flex flex-row md:flex-col gap-6 z-30"
      >
        {[FaFacebookF, FaInstagram, FaTiktok, FaWhatsapp, MdEmail].map((Icon, i) => (
          <motion.a
            whileHover={{ scale: 1.2, rotate: -5 }}
            key={i}
            href="#"

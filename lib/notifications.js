...
export async function notifyAdmins(db, { eventType, message, userId = null, userName = null, userEmail = null, userImage = null, tripId = null, messageId = null }) {
  try {
    ...
    await Promise.all(admins.map((admin) => db.query(
      `INSERT INTO notifications
        (id, admin_id, event_type, message, user_id, user_name, user_email, user_image, trip_id, created_at, is_read, message_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), 0, ?)`,
      [uuidv4(), admin.id, eventType, String(message || "New WasetTravel activity"), userId || "00000000-0000-0000-0000-000000000000", userName || "Traveler", userEmail || "", userImage || "/default-avatar.png", tripId || "00000000-0000-0000-0000-000000000000", messageId || uuidv4()],
    )));
